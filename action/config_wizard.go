...
// ConfigWizard reads configuration from a gopkg.yaml file and attempts to suggest
...
func ConfigWizard(base string) {
	...
	if err != nil {
		msg.Info("Unable to find a gopkg.yaml file. Would you like to create one now? Yes (Y) or No (N)")
		...
		if bres {
			...
			if err := conf.WriteFile(gopkgfile); err != nil {
				msg.Die("Could not save %s: %s", gopkgfile, err)
			}
		} else {
	...
	if changes > 0 {
		...
		if dres {
			msg.Info("Writing updates to configuration file (%s)", gopkgfile)
			if err := conf.WriteFile(gopkgfile); err != nil {
				msg.Die("Could not save %s: %s", gopkgfile, err)
			}
			msg.Info("You can now edit the gopkg.yaml file.:")
			msg.Info("--> For more information on versions and ranges see https://gopkg.sh/docs/versions/")
			msg.Info("--> For details on additional metadata see https://gopkg.sh/docs/gopkg.yaml/")
		} else {
...
func wizardAskRange(ver *semver.Version, d *cfg.Dependency) string {
	...
	msg.Info("For more information on Gopkg versions and ranges see https://gopkg.sh/docs/versions")
