...
func writeLock(conf, confcopy *cfg.Config, base string) {
	...
	if err := lock.WriteFile(filepath.Join(base, gpath.LockFile)); err != nil {
		msg.Die("Failed to write gopkg lock file: %s", err)
	}
}
		...
		if conf.HasDependency(root) {
			...
			if subpkg != "" {
				...
				if dep.HasSubpackage(subpkg) {
					if !moved {
						msg.Warn("--> Package %q is already in gopkg.yaml. Skipping", name)
					}
				} else {
				...
				msg.Warn("--> Package %q is already in gopkg.yaml. Skipping", root)
			}
			continue
		}

		if conf.HasIgnore(root) {
			msg.Warn("--> Package %q is set to be ignored in gopkg.yaml. Skipping", root)
			continue
		}
