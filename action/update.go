...
// Update updates repos and the lock file from the main gopkg yaml.
func Update(installer *repo.Installer, skipRecursive, stripVendor bool) {
	...
	// should be added to the gopkg.yaml file. See issue #193.
	...
	if !skipRecursive {
		...
		if wl {
			...
			msg.Info("Versions did not change. Skipping gopkg.lock update.")
		}
