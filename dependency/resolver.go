	...
	for e := queue.Front(); e != nil; e = e.Next() {
		...
		if err != nil && strings.HasPrefix(err.Error(), "found packages ") {
			...
			if strings.HasPrefix(errStr, "no buildable Go source") {
				...
				msg.Err("version in your gopkg.yaml that works for all packages that share this")
				msg.Err("dependency.")
			} else {
				r.hadError[dep] = true
				msg.Err("Error scanning %s: %s", dep, err)
...
func (r *Resolver) FindPkg(name string) *PkgInfo {
	...
	// No cache:
	// gopkg -y etcd.yaml list  0.27s user 0.19s system 85% cpu 0.534 total
	// With cache:
	// gopkg -y etcd.yaml list  0.22s user 0.15s system 85% cpu 0.438 total
