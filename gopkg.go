...
import (
	...
	"github.com/Khulnasoft-lab/gopkg/action"
...
const usage = `Vendor Package Management for your Go projects.
   ...
   Each project should have a 'gopkg.yaml' file in the project directory. Files
   ...
   For more details on the 'gopkg.yaml' files see the documentation at
   https://gopkg.sh/docs/gopkg.yaml
`
...
func commands() []cli.Command {
   ...
   To fetch the dependencies you may run 'gopkg install'.`,
			Flags: []cli.Flag{
				cli.BoolFlag{
					Name:  "skip-import",
		...
		{
			Name:  "get",
			Usage: "Install one or more packages into `vendor/` and add dependency to gopkg.yaml.",
   ...
   folder.`,
		...
		},
		{
			Name:      "remove",
			ShortName: "rm",
			Usage:     "Remove a package from the gopkg.yaml file, and regenerate the lock file.",
			Description: `This takes one or more package names, and removes references from the gopkg.yaml file.
   This will rebuild the gopkg lock file re-resolving the depencies.`,
			Flags: []cli.Flag{
				cli.BoolFlag{
					Name:  "delete,d",
		...
		},
		{
			...
			Description: `Read the gopkg.yaml file and print the name given on the 'package' line.`,
			Action: func(c *cli.Context) error {
				action.Name()
				return nil
			},
...
Example:
   ...
   be installed. When there is a gopkg.yaml file defining the dependencies but
   no lock file (gopkg.lock) the dependencies are installed using the "update"
   command and a gopkg.lock file is generated pinning all dependencies. If a
   gopkg.lock file is already present the dependencies are installed or updated
   from the lock file.`,
			Flags: []cli.Flag{
				cli.BoolFlag{
					Name:   "delete",
   ...
   in the gopkg.yaml file. When no rules exist the tip of the default branch
   is used. For more details see https://gopkg.sh/docs/gopkg.yaml

   If a dependency has a gopkg.yaml file, update will read that file and
   ...
   folder.`,
			Flags: []cli.Flag{
				...
				cli.BoolFlag{
					Name:  "no-recursive, quick",
					Usage: "Disable updating dependencies' dependencies. Only update things in gopkg.yaml.",
				},
				cli.BoolFlag{
					Name:  "force",
					Usage: "If there was a change in the repo or VCS switch to new one. Warning, changes will be lost.",
   ...
   It does not use the gopkg.yaml. Instead, it inspects the code to determine what packages are
   imported.
   ...
   For example, given a project with the following gopkg.yaml:
   ...
   Then running the following commands:
       ...
       gopkg info -f "License: %l"
          prints 'License: MIT'

       gopkg info -f "%n - %d - %h - %l"
...
// Get the path to the gopkg.yaml file.
//
...
func gopkgfile(c *cli.Context) string {
	path := c.String("file")
	if path == "" {
		// For now, we construct a basic assumption. In the future, we could
		// traverse backward to see if a gopkg.yaml exists in a parent.
		path = "./gopkg.yaml"
	}
